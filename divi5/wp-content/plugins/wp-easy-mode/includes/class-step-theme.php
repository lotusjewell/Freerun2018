...
final class Step_Theme extends Step {
					...
					<?php if ( 'standard' === wpem_get_site_type() ) { ?>
						<ul class="section-titles">
							<li class="section-title active" data-section="header-image">
								<h3><?php esc_html_e( 'Header Image', 'wpem' ); ?></h3>
							</li>
							<li class="section-title" data-section="color-scheme">
								<h3><?php esc_html_e( 'Color Scheme', 'wpem' ); ?></h3>
							</li>
						</ul>
					<?php } ?>
	...
	public function color_scheme_list() {
		...
		<div id="color_scheme_selection">
			...
			$color_scheme_option = get_option( 'wpem_color_scheme', 'default' );

			foreach ( $color_schemes as $color_scheme_name => $color_scheme_data ) :

				$color_scheme_name = $color_scheme_data['label'];
				...
				$selected_class = ( strtolower( $color_scheme_name ) === $color_scheme_option ) ? ' selected' : '';

				?>
				<div class="color-option<?php echo esc_attr( $selected_class ); ?>">
					<input name="wpem_color_scheme" type="radio" value="<?php echo esc_attr( strtolower( $color_scheme_name ) ); ?>" class="tog" <?php checked( $color_scheme_option, strtolower( $color_scheme_name ) ); ?> style="display:none;">
					<input type="hidden" class="css_url" value="">
					<input type="hidden" class="icon_colors" value="color_scheme_name">
					<label for="admin_color_fresh"><?php echo ( 'Default' == $color_scheme_name ) ? 'Theme' : esc_html( $color_scheme_name ); ?></label>
						...
						<tbody>
							<tr>
								...
								<?php if ( 'Default' === $color_scheme_name ) { ?>
									...
									<td style="width:100%;"><strong><?php _e( 'Default', 'wpem' ); ?></strong></td>
